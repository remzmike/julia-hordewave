# MK: 7/7/2021: added onkeyup stuff
...
function handleKeyPress(g::Game, e, t)
    ...
    if (t == SDL2.KEYDOWN)
        #println("********* DOWN >>> ", Keys.Key(keySym))
        push!(g.keyboard, keySym)
        Base.invokelatest(g.onkey_function, g, Keys.Key(keySym), keyMod)
    elseif (t == SDL2.KEYUP)
        #println("********* UP >>> ", Keys.Key(keySym))
        Base.invokelatest(g.onkeyup_function, g, Keys.Key(keySym), keyMod)
...
function initgame(jlf::String)
    ...
    g.onkeyup_function = getfn(game_module, :on_key_up, 3)
